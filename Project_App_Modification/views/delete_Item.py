...
from Project_App_Allocation.models import Final_Allocation, Allocation_Number
...
def view_final_allocation(request, allocation_id):
    ...
    for i in range(1, 10):
        ...
        if filter_type == "pbs":
            ...
            allocations = allocations.filter(project__projectId__iexact=filter_value)
    ...
    return render(
        request,
        "Project_Templates/Project_App_Modification/view_and_delete_Item.html",
        {
...
def delete_final_allocation_entry(request, allocation_id):
    ...
    if request.method == "POST":
        ...
        messages.success(request, "Project Allocation entry deleted successfully.")
        return redirect("Project_App_Modification:view_final_allocation", allocation_id=allocation_no_id)
